...
Candy.View.Template = (function(self){
	...
	self.Chat = {
		...
		tab: '<li class="roomtype-{{roomType}}" data-roomjid="{{roomJid}}" data-roomtype="{{roomType}}"><span class="label">{{#privateUserChat}}@{{/privateUserChat}}{{name}}</span><span href="#" class="transition"></span><a href="#" class="close">\u00D7</a><small class="unread"></small></li>',
	...
	self.Room = {
		...
		form: '<div class="message-form-wrapper"></div><form method="post" class="message-form"><input name="message" class="field" type="text" autocomplete="off" maxlength="1000" x-webkit-speech /><input type="submit" class="submit" name="submit" value="{{_messageSubmit}}" /></form>'
	};
	...
	self.Message = {
		pane: '<div class="message-pane-wrapper"><dl class="message-pane"></dl></div>',
                item: '<dt>{{time}}</dt><dd><span class="name">&lt;<a href="#">{{displayName}}</a>&gt;</span> {{{message}}}</dd>',
                action: '<dt>{{time}}</dt><dd>** <span class="name"><a href="#">{{displayName}}</a></span> {{{message}}}</dd>',
	};
